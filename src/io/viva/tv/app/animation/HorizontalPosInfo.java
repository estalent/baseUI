...
public class HorizontalPosInfo {
	...
	private static PosInfo SelectorAlphaChange(float originX, float originY, float destX, float destY, float radio, float startX, float endX, float startY, float endY, float startScale, float endScale, float startAlpha, float endAlpha) {
		...
		TransInfoKey interPolateStart = new TransInfoKey((startX + (endX - startX) * 0.4F) * radio - originX + destX, startY * radio - originY + destY, startScale + 0.4F * (endScale - startScale), startScale + 0.4F * (endScale - startScale), 0.3F, 0.4F);

		TransInfoKey interPolateEnd = new TransInfoKey((startX + 0.6F * (endX - startX)) * radio - originX + destX, endY * radio - originY + destY, startScale + 0.6F * (endScale - startScale), startScale + 0.6F * (endScale - startScale), 0.3F, 0.6F);
	...
	public static PosInfo getAnimStablePos(float originX, float originY, float destX, float destY, int moveDirection, int viewIndex, int targetImageWidth) {
		...
		posInfo = new PosInfo(startX * radio - originX + destX, endX * radio - originX + destX, startY * radio - originY + destY, endY * radio - originY + destY, startScale, endScale, startScale, endScale, startAlpha, endAlpha);
	...
	public static PosInfo getAnimUnstablePos(float originX, float originY, float destX, float destY, int moveDirection, int viewSelectIndex, int viewIndex, int targetImageWidth) {
		...
		if (moveDirection == 1) {
			...
			posInfo = new PosInfo(endX * radio - originX + destX, startX * radio - originX + destX, endY * radio - originY + destY, startY * radio - originY + destY, endScale, startScale, endScale, startScale, endAlpha, startAlpha);
		}
	...
	public static PosInfo getUniversalAnimatorPosInfo(float originX, float originY, float destX, float destY, int targetImageWidth, int adapterViewIndex, int count, int selectedPosition, int newSelectedPosition) {
		if (count < 6) {
			return null;
		}
	...
	private static TransInfoKey pointToStatus(float key, int aniStatus, int viewIndex, BasePositionInfo baseInfo) {
		...
		if (aniStatus != ANI_STATUS_A6) {
			...
			TransInfoKey keyFrame = new TransInfoKey(info.X * radio - baseInfo.originX + baseInfo.destX, info.Y * radio - baseInfo.originY + baseInfo.destY, info.Scale, info.Scale, info.Alpha, key);
	...
	public static PosInfo getUniversalSelectorPosInfo(float originX, float originY, float destX, float destY, int targetImageWidth, int count, int selectedPosition, int newSelectedPosition) {
		if (count < 6) {
			return null;
		}
	...
	private static void SelectorAlphaChange(PosInfo posInfo, TransInfoKey startKey, TransInfoKey endKey, BasePositionInfo baseInfo) {
		...
		TransInfoKey interPolateStart = new TransInfoKey(startTransInfo.x + (endTransInfo.x - startTransInfo.x) * 0.4F, startTransInfo.y, startTransInfo.scaleX + 0.4F * (endTransInfo.scaleX - startTransInfo.scaleX), startTransInfo.scaleY + 0.4F
				* (endTransInfo.scaleY - startTransInfo.scaleY), 0.3F, 0.4F);

		TransInfoKey interPolateEnd = new TransInfoKey(startTransInfo.x + 0.6F * (endTransInfo.x - startTransInfo.x), endTransInfo.y, startTransInfo.scaleX + 0.6F * (endTransInfo.scaleX - startTransInfo.scaleX), startTransInfo.scaleY + 0.6F
				* (endTransInfo.scaleY - startTransInfo.scaleY), 0.3F, 0.6F);
	...
	public static TransInfo getUniversalInitialPosInfo(float originX, float originY, float destX, float destY, int targetImageWidth, int count, int selectedPosition, int adapterViewIndex) {
		if (count < 6) {
			return null;
		}
