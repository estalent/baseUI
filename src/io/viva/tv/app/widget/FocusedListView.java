...
public class FocusedListView extends ListView implements FocusedBasePositionManager.PositionInterface {
	...
	public void setSelection(int paramInt) {
		...
		LOG.i("ccdd", true, new StringBuilder().append("setSelection:").append(paramInt).append(",top:").append(i).append(",getSelectedView().getTop():").append(getSelectedView() != null ? new StringBuilder().append(getSelectedView().getTop()).append("").toString() : "null")
				.append(",this.getPaddingTop():").append(getPaddingTop()).append(",getCount():").append(getCount()).toString());
	}
	...
	public View getSelectedView() {
		...
		LOG.i("FocusedListView", true, new StringBuilder().append("getSelectedView mcurrentPosition:").append(this.mCurrentPosition).append(",firstVisiblePosition:").append(getFirstVisiblePosition()).toString());
		return localView;
	}
	...
	protected void onFocusChanged(boolean paramBoolean, int paramInt, Rect paramRect) {
		LOG.i("ccdd", true, new StringBuilder().append("onFocusChanged,gainFocus:").append(paramBoolean).append(", mCurrentPosition = ").append(this.mCurrentPosition).append(", child count = ").append(getChildCount()).append(",focusMode:").append(this.focusPositionMode)
				.toString());
		...
		if (!paramBoolean) {
			...
			LOG.i("FocusedListView", true, new StringBuilder().append("onfocus setSelection:").append((this.mCurrentPosition > -1) && (this.mCurrentPosition < getCount()) ? this.mCurrentPosition : 0).toString());
	...
	public boolean onKeyDown(int paramInt, KeyEvent paramKeyEvent) {
		...
		synchronized (this) {
			if ((System.currentTimeMillis() - this.mKeyTime <= this.KEY_INTERVEL) || (this.mPositionManager.getState() == 1) || (isScrolling())) {
				LOG.d("lingdang", true, new StringBuilder().append("KeyInterval:").append(System.currentTimeMillis() - this.mKeyTime <= this.KEY_INTERVEL).append(", getState():").append(this.mPositionManager.getState() == 1 ? "drawing" : "idle").append(", isScrolling: = ")
						.append(isScrolling()).toString());
