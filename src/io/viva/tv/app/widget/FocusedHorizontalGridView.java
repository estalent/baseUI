...
public class FocusedHorizontalGridView extends HorizontalGridView implements FocusedBasePositionManager.PositionInterface {
	...
	private AbsHorizontalListView.OnScrollListener mOnScrollListener = new AbsHorizontalListView.OnScrollListener() {
		public void onScrollStateChanged(AbsHorizontalListView paramAnonymousAbsHorizontalListView, int paramAnonymousInt) {
			...
			case 2:
				...
				break;
			case 0:
				Log.d("FocusedHorizontalGridView", "onScrollStateChanged idle mNeedScroll = " + FocusedHorizontalGridView.this.mNeedScroll + ", mSpinnerPadding.left = " + FocusedHorizontalGridView.this.mSpinnerPadding.left);
				if (FocusedHorizontalGridView.this.mNeedScroll) {
					...
					if ((FocusedHorizontalGridView.this.mHorizontalMode == 2) && (FocusedHorizontalGridView.this.mScrollList.size() <= 0)) {
						...
						FocusedHorizontalGridView.this.setPadding(FocusedHorizontalGridView.this.mPaddingLeft, FocusedHorizontalGridView.this.mSpinnerPadding.top, FocusedHorizontalGridView.this.mSpinnerPadding.right, FocusedHorizontalGridView.this.mSpinnerPadding.bottom);
	...
	public void dispatchDraw(Canvas paramCanvas) {
		Log.i("FocusedHorizontalGridView", new StringBuilder().append("dispatchDraw child count = ").append(getChildCount()).append(", mOutsieScroll = ").append(this.mOutsieScroll).toString());
	...
	protected void onFocusChanged(boolean paramBoolean, int paramInt, Rect paramRect) {
		Log.d("FocusedHorizontalGridView", new StringBuilder().append("onFocusChanged,gainFocus:").append(paramBoolean).append(", mCurrentPosition = ").append(this.mCurrentPosition).append(", child count = ").append(getChildCount()).toString());
	...
	private void focusInit() {
		if (this.mIsFocusInit)
			return;
		Log.d("FocusedHorizontalGridView", new StringBuilder().append("focusInit mCurrentPosition = ").append(this.mCurrentPosition).append(", getSelectedItemPosition() = ").append(getSelectedItemPosition()).toString());
		...
		if (!hasFocus()) {
			...
			if (this.focusPositionMode == FOCUS_ITEM_AUTO_SEARCH) {
				...
				Log.i("FocusedHorizontalGridView", new StringBuilder().append("onfocus setSelection:").append((this.mCurrentPosition > -1) && (this.mCurrentPosition < getCount()) ? this.mCurrentPosition : 0).toString());
	...
	public View getSelectedView() {
		...
		if ((i < getFirstVisiblePosition()) || (i > getLastVisiblePosition())) {
			Log.w("FocusedHorizontalGridView",
					new StringBuilder().append("getSelectedView mCurrentPosition = ").append(this.mCurrentPosition).append(", getFirstVisiblePosition() = ").append(getFirstVisiblePosition()).append(", getLastVisiblePosition() = ").append(getLastVisiblePosition()).toString());
			return null;
		}
		int j = i - getFirstVisiblePosition();
		View localView = getChildAt(j);
		Log.d("FocusedHorizontalGridView",
				new StringBuilder().append("getSelectedView getSelectedView: mCurrentPosition = ").append(this.mCurrentPosition).append(", indexOfView = ").append(j).append(", child count = ").append(getChildCount()).append(", getFirstVisiblePosition() = ")
	...
	public boolean onKeyDown(int paramInt, KeyEvent paramKeyEvent) {
		...
		Log.d("FocusedHorizontalGridView", new StringBuilder().append("onKeyDown keyCode = ").append(paramInt).append(", child count = ").append(getChildCount()).append(", mCurrentPosition = ").append(this.mCurrentPosition).toString());
		synchronized (this) {
			if (this.mPositionManager.getState() == 1) {
				Log.d("FocusedHorizontalGridView", new StringBuilder().append("onKeyDown KyeInterval = ").append(System.currentTimeMillis() - this.mKeyTime).append(", getState() = ").append(this.mPositionManager.getState()).append(", isScrolling() = ").append(isScrolling())
						.toString());
				return true;
			}
	...
	public boolean arrowScroll(int paramInt) {
		...
		boolean bool2 = true;
		Log.d("FocusedHorizontalGridView", new StringBuilder().append("arrowScroll this.mLastPosition = ").append(this.mLastPosition).append(", this.mCurrentPosition = ").append(this.mCurrentPosition).append(", lastPosition = ").append(i).toString());
		if (localView1 != null)
	...
	void arrowSmoothScrollSingle(int paramInt) {
		...
		endFling();
		int i = getCurrentX();
		Log.e("FocusedHorizontalGridView", new StringBuilder().append("arrowSmoothScroll currY = ").append(i).append(", mCurrentPosition = ").append(this.mCurrentPosition).append(", isScrolling = ").append(bool).toString());
		if (this.mScrollX < 0)
			i -= 2147483647;
		Log.e("FocusedHorizontalGridView", new StringBuilder().append("arrowSmoothScroll currY = ").append(i).append(", mScrollY = ").append(this.mScrollX).append(", scrollBy = ").append(paramInt).toString());
		this.mScrollX -= i;
	...
	void checkSelection() {
		...
		while (i <= getLastVisiblePosition()) {
			...
			if (null != localView) {
				Log.d("FocusedHorizontalGridView",
						new StringBuilder().append("arrowSmoothScroll child.getTop() = ").append(localView.getTop()).append(", child.getBottom() = ").append(localView.getBottom()).append(", this.getTop() = ").append(getTop()).append(", this.getBottom() = ").append(getBottom())
								.append(", pos = ").append(i).toString());
				if ((localView.getTop() > getTop()) && (localView.getBottom() < getBottom())) {
					Log.d("FocusedHorizontalGridView", new StringBuilder().append("arrowSmoothScroll child.getTop() = ").append(localView.getTop()).append(", child.getBottom() = ").append(localView.getBottom()).append(", pos = ").append(i).toString());
	...
	private boolean checkFocusPosition() {
		...
		Log.i("FocusedHorizontalGridView", new StringBuilder().append("checkFocusPosition:").append(this.mPositionManager.getCurrentRect()).append(",").append(hasFocus()).append(",").append(localRect).append(",").append(isShown()).toString());
		if ((null == this.mPositionManager.getCurrentRect()) || (!hasFocus()) || (null == localRect) || (!isShown()) || (this.mPositionManager.getContrantNotDraw()))
			return false;
		return (Math.abs(localRect.left - this.mPositionManager.getCurrentRect().left) > 5) || (Math.abs(localRect.right - this.mPositionManager.getCurrentRect().right) > 5) || (Math.abs(localRect.top - this.mPositionManager.getCurrentRect().top) > 5)
				|| (Math.abs(localRect.bottom - this.mPositionManager.getCurrentRect().bottom) > 5);
	}
		...
		public Rect getDstRectAfterScale(boolean paramBoolean) {
			...
			if (FocusedHorizontalGridView.this.isScrolling()) {
				...
				if (FocusedHorizontalGridView.this.mScrollDirection == 1) {
					...
					if (localRect1.left + localView2.getWidth() * getItemScaleXValue() >= FocusedHorizontalGridView.this.getRight() - FocusedHorizontalGridView.this.getPaddingRight())
