...
public class FocusedHorizontalListView extends HorizontalListView implements FocusedBasePositionManager.PositionInterface {
	...
	protected void onFocusChanged(boolean paramBoolean, int paramInt, Rect paramRect) {
		Log.i("FocusedHorizontalListView", new StringBuilder().append("onFocusChanged,gainFocus:").append(paramBoolean).append(", mCurrentPosition = ").append(this.mCurrentPosition).append(", child count = ").append(getChildCount()).toString());
		...
		synchronized (this) {
			if ((System.currentTimeMillis() - this.mKeyTime <= this.KEY_INTERVEL) || (this.mPositionManager.getState() == 1)) {
				Log.d("FocusedHorizontalListView",
						new StringBuilder().append("onKeyDown KeyInterval:").append(System.currentTimeMillis() - this.mKeyTime <= this.KEY_INTERVEL).append(", getState():").append(this.mPositionManager.getState() == 1 ? "drawing" : "idle").append(", isScrolling: = ").toString());
				return true;
			}
			this.mKeyTime = System.currentTimeMillis();
		}
	...
	void arrowSmoothScroll(int paramInt) {
		...
		endFling();
		int i = getActualX();
		Log.e("FocusedHorizontalListView", new StringBuilder().append("arrowSmoothScroll currX = ").append(i).append(", mCurrentPosition = ").append(this.mCurrentPosition).append(", isScrolling = ").append(bool).append(", scrollBy = ").append(paramInt).toString());
		if (this.mScrollX < 0)
			i -= 2147483647;
		Log.e("FocusedHorizontalListView", new StringBuilder().append("arrowSmoothScroll currX = ").append(i).append(", mScrollX = ").append(this.mScrollX).append(", scrollBy = ").append(paramInt).toString());
		this.mScrollX -= i;
		...
		for (int i = this.mCurrentPosition; i <= getLastVisiblePosition(); i++) {
			...
			if (null != localView) {
				...
				if ((localView.getLeft() >= getLeft()) && (localView.getRight() <= getRight())) {
					Log.d("FocusedHorizontalListView", new StringBuilder().append("arrowSmoothScroll child.getTop() = ").append(localView.getTop()).append(", child.getBottom() = ").append(localView.getBottom()).append(", pos = ").append(i).toString());
		...
		public Rect getDstRectAfterScale(boolean paramBoolean) {
			...
			if (FocusedHorizontalListView.this.isScrolling()) {
				...
				if (FocusedHorizontalListView.this.mScrollDirection == 1) {
					...
					if (localRect1.left + localView2.getWidth() * getItemScaleXValue() >= FocusedHorizontalListView.this.getRight() - FocusedHorizontalListView.this.getPaddingRight())
