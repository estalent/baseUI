...
public class FocusedGridView extends GridView implements FocusedBasePositionManager.PositionInterface, FlingManager.FlingCallback {
	...
	public void dispatchDraw(Canvas paramCanvas) {
		LOG.d("FocusedGridView", true, "dispatchDraw child count = " + getChildCount() + ", first position = " + getFirstVisiblePosition() + ", last posititon = " + getLastVisiblePosition());
	...
	public void getFocusedRect(Rect paramRect) {
		Log.i("FocusedGridView", "getFocusedRect mCurrentPosition" + this.mCurrentPosition + ",getFirstVisiblePosition:" + getFirstVisiblePosition() + ",getLastVisiblePosition:" + getLastVisiblePosition());
	...
	public View getSelectedView() {
		...
		if ((i < getFirstVisiblePosition()) || (i > getLastVisiblePosition())) {
			Log.w("FocusedGridView", "getSelectedView mCurrentPosition = " + this.mCurrentPosition + ", getFirstVisiblePosition() = " + getFirstVisiblePosition() + ", getLastVisiblePosition() = " + getLastVisiblePosition());
			return null;
		}
		if (checkHeaderPosition())
		...
		LOG.d("FocusedGridView", true, "getSelectedView getSelectedView: mCurrentPosition = " + this.mCurrentPosition + ", indexOfView = " + j + ", child count = " + getChildCount() + ", getFirstVisiblePosition() = " + getFirstVisiblePosition() + ", getLastVisiblePosition() = "
	...
	public boolean onKeyDown(int paramInt, KeyEvent paramKeyEvent) {
		...
		synchronized (this) {
			if (this.mPositionManager.getState() == 1) {
				Log.w("FocusedGridView", "onKeyDown KyeInterval = " + (System.currentTimeMillis() - this.mKeyTime) + ", getState() = " + this.mPositionManager.getState() + ", isScrolling() = " + isScrolling());
				return true;
			}
		...
		if ((getSelectedView() != null) && (getSelectedView().onKeyDown(paramInt, paramKeyEvent)))
			return true;
		if ((hasHeader())
				&& ((((this.mCurrentPosition + 1) / getNumColumns() == 1) && ((this.mCurrentPosition + 1) % getNumColumns() == 0) && (22 == paramInt)) || ((this.mCurrentPosition / getNumColumns() == 1) && (this.mCurrentPosition % getNumColumns() == 0) && (21 == paramInt))))
			return true;
		switch (paramInt) {
		case 19:
			if (!arrowScroll(33)) {
	...
	public boolean arrowScroll(int paramInt) {
		...
		if (checkHeaderPosition(this.mCurrentPosition)) {
			...
			LOG.d("FocusedGridView", true, "arrowScroll header mCurrentPosition = " + this.mCurrentPosition + ", mHeaderPosition = " + this.mHeaderPosition + ", mHeaderSelected = " + this.mHeaderSelected);
	...
	public void smoothScrollBy(int paramInt1, int paramInt2) {
		...
		if ((paramInt1 == 0) || (getAdapter().getCount() == 0) || (j == 0) || ((i == 0) && (getChildAt(0).getTop() == m) && (paramInt1 < 0)) || ((k == getAdapter().getCount() - 1) && (getChildAt(j - 1).getBottom() == n) && (paramInt1 > 0))) {
			this.mFlingManager.endFling();
		} else {
	...
	class FocusedGridPositionManager extends FocusedBasePositionManager {
		...
		public Rect getDstRectBeforeScale(boolean paramBoolean) {
			...
			LOG.d("FocusedBasePositionManager", true, "getDstRectBeforeScale imgView.top = " + localRect1.top + ", imgView.bottom = " + localRect1.bottom + ", gridiew.top = " + FocusedGridView.this.getTop() + ", gridview.bottom = " + FocusedGridView.this.getBottom());
			if (paramBoolean) {
