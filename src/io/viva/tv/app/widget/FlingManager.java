...
class FlingManager implements Runnable {
	...
	void startScroll(int paramInt1, int paramInt2) {
		LOG.i("FlingManager", true, "FlingRunnable startScroll distance = " + paramInt1 + ", duration");
	...
	boolean trackMotionScroll(int paramInt1, int paramInt2) {
		...
		if ((bool1) || (bool2)) {
			Log.w("FlingManager", "trackMotionScroll cannotScrollDown = " + bool1 + ", cannotScrollUp = " + bool2);
			return paramInt2 != 0;
		}
		...
		if (bool3) {
			...
			int i9 = this.mGridView.getHeight() - paramInt2;
			if ((getGroupFlags() & this.mFlingCallback.getClipToPaddingMask()) == this.mFlingCallback.getClipToPaddingMask())
				i9 -= localRect.bottom;
	...
	void focusedReportScrollStateChange(int paramInt) {
		try {
			Class localClass = Class.forName("android.widget.AbsListView");
	...
	public static abstract interface FlingCallback {
		...
		public abstract boolean flingOverScrollBy(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5, int paramInt6, int paramInt7, int paramInt8, boolean paramBoolean);
