...
public class CoverFlowOpenGL extends GLSurfaceView implements GLSurfaceView.Renderer {
	...
	private void showOverlayView() {
		FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(this.mCenterBound.width(), this.mCenterBound.height());
		params.setMargins(this.mCenterBound.left, this.mCenterBound.top, this.mCenterBound.right, this.mCenterBound.bottom);
		this.mFocusRect.setLayoutParams(params);
	}
	...
	private void startAnimation(double speed) {
		...
		this.mStartSpeed = ((float) Math.sqrt(Math.abs(nearest - this.mStartOffset) * 10.0D * 2.0D));
	...
	private void updateAnimationAtElapsed(float elapsed) {
		...
		float delta = Math.abs(this.mStartSpeed) * elapsed - 10.0F * elapsed * elapsed / 2.0F;
	...
	public void onSurfaceChanged(GL10 gl, int w, int h) {
		...
		this.mTouchRect = new RectF(w / 2 - imagew, h / 2 - imageh, w / 2 + imagew, h / 2 + imageh);
	...
	private void calcCenterRect(GL10 gl) {
		...
		ProjectToScreen(gl, this.mVerticesBuffer.get(0), this.mVerticesBuffer.get(1), this.mVerticesBuffer.get(2), ret);
		this.mCenterBound.left = ((int) ret[0] - 5);
		this.mCenterBound.top = ((int) ret[1] - 5);
		ProjectToScreen(gl, this.mVerticesBuffer.get(9), this.mVerticesBuffer.get(10), this.mVerticesBuffer.get(11), ret);
	...
	private void drawTile(int position, float off, GL10 gl) {
		...
		if ((fcr != null) && (fcr.mTexture != 0)) {
			...
			gl.glRotatef((float) (-f * this.mAngle * 180.0F / 3.141592653589793D), 0.0F, 1.0F, 0.0F);
	...
	private CoverFlowRecord getTileAtIndex(int position, GL10 gl) {
		synchronized (this) {
			CoverFlowRecord fcr = (CoverFlowRecord) this.mCache.objectForKey(Integer.valueOf(position));
			if (fcr == null) {
		...
		if (this.mGLContext != null)
			new Thread(new Runnable() {
				public void run() {
					...
					int max = CoverFlowOpenGL.this.mListener.getCount(CoverFlowOpenGL.this);
					int end = endIndex > max ? max : endIndex;

					for (int i = start; (i < end) && (!CoverFlowOpenGL.this.mStopBackgroundThread); i++)
						CoverFlowOpenGL.this.getTileAtIndex(i, CoverFlowOpenGL.this.mGLContext);
				}
			}).run();
	}
	...
	public static abstract interface CoverFlowListener {
		...
		public abstract Bitmap getImage(CoverFlowOpenGL paramCoverFlowOpenGL, int paramInt);

		public abstract void tileOnTop(CoverFlowOpenGL paramCoverFlowOpenGL, int paramInt);

		public abstract void topTileClicked(CoverFlowOpenGL paramCoverFlowOpenGL, int paramInt);
	}
		...
		class MyThread implements Runnable {
			...
			public void run() {
				synchronized (CoverFlowOpenGL.this.mutex) {
					...
					Canvas canvas = CoverFlowOpenGL.FocusView.this.holder.lockCanvas(null);

					Drawable border = CoverFlowOpenGL.FocusView.this.getContext().getResources().getDrawable(2114191425);

					border.setBounds(0, 0, CoverFlowOpenGL.this.mCenterBound.width(), CoverFlowOpenGL.this.mCenterBound.height());
					border.draw(canvas);

					CoverFlowOpenGL.FocusView.this.holder.unlockCanvasAndPost(canvas);
				}
			}
		}
	}
