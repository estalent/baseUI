...
public abstract class AbsCoverFlowAdapter extends BaseAdapter {
	...
	protected final DataCache<Integer, Bitmap> mCache = new DataCache(20);
	...
	public final synchronized ImageView getView(int paramInt, View paramView, ViewGroup paramViewGroup) {
		...
		BitmapShader localBitmapShader = new BitmapShader((Bitmap) localObject, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
		Paint localPaint = localBitmapDrawable.getPaint();
		localPaint.setAntiAlias(true);
		this.matrix.setScale(this.width / ((Bitmap) localObject).getWidth(), this.height / ((Bitmap) localObject).getHeight(), 0.5F, 0.5F);
